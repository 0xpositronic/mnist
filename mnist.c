...
float *he_init(int rows, int cols) {
  ...
  float scale = sqrt(12.00f / (rows + cols));
  srand(42);
  ...
  case SOFTMAX: {
    ...
    for (int i = 0; i < batch_size; i++) {
      ...
      for (int j = 0; j < layer->out_size; j++) {
        ...
        layer->H[i * layer->out_size + j] = val;
        sum += val;
      }
      for (int j = 0; j < layer->out_size; j++) {
...
float *layer_backward(Layer *layer, float *dH_above, int batch_size) {
  ...
  case DENSE: {
    ...
    layer->dW = (float *)malloc(sizeof(float) * layer->in_size * layer->out_size);
    layer->dB = (float *)malloc(sizeof(float) * layer->out_size);

    float *W_T = transpose(layer->W, layer->in_size, layer->out_size);
    float *X_T = transpose(layer->X, batch_size, layer->in_size);
    
    matmul(dH_above, W_T, layer->dH, batch_size, layer->out_size, layer->in_size);
    matmul(X_T, dH_above, layer->dW, layer->in_size, batch_size, layer->out_size);
...
float *forward(Network *net, float *input, int batch_size, int features) {
...
float *get_prediction(float *h, int batch_size, int n_classes) {
  float *predictions = (float *)malloc(sizeof(float) * batch_size);
  for (int i = 0; i < batch_size; i++) {
    float pred = 0;
    int max = 0;
    for (int j = 0; j < n_classes; j++) {
...
int main() {
  ...
  for (int i = 0; i < iters; i++) {
    float *h = forward(&net, batch_data, n_images, image_size);
    float loss = calc_loss(h, batch_labels, n_images, n_classes);
    backward(&net, loss, batch_labels, n_images);
  ...
  float *test_data = read_mnist_images(DATA_DIR "t10k-images-idx3-ubyte", &test_n_images, &test_image_size, 1);
  float *test_labels = read_mnist_labels(DATA_DIR "t10k-labels-idx1-ubyte", test_n_images);

  float *test_h = forward(&net, test_data, test_n_images, image_size);
  float *preds = get_prediction(test_h, test_n_images, n_classes);
